...
/* ── Exercise 5: Zoom-out 256x256 → 512x512 검정 배경에 translation ── */
...
uint8** translation(uint8** small, int tx, int ty) {
    int y, x, nx, ny;
...
int main(void) {
    ...
    uint8** output = translation(small, 50, 50);
    write_bmp("output_5_translation.bmp", output, 512, 512);
    free_2d(lena);
    free_2d(small);
    free_2d(output);
    printf("output_5_translation.bmp 생성 완료\n");
    return 0;
}
