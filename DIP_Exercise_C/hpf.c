...
int main(void) {
    ...
    /* ideal HPF: zero out components within radius (= LPF complement) */
    int cx = N/2, cy = N/2;
    for (int i = 0; i < N; i++) {
        for (int j = 0; j < N; j++) {
    ...
    /* de-center and take absolute value */
    ...
    for (int i = 0; i < N; i++) {
        ...
        for (int j = 0; j < N; j++) {
            double sign    = ((i+j) % 2 == 0) ? 1.0 : -1.0;
            real_out[i][j] = fabs(creal(F[i][j]) * sign);
            if (real_out[i][j] > vmax) vmax = real_out[i][j];
        }
    }

    /* normalize by max abs value -> 0(검정,평탄) ~ 255(밝음,엣지) */
    unsigned char **out = malloc(N * sizeof(unsigned char *));
    for (int i = 0; i < N; i++) {
        out[i] = malloc(N);
        for (int j = 0; j < N; j++) {
            double norm = (vmax > 0) ? (real_out[i][j] / vmax * 255.0) : 0.0;
    ...
    write_bmp("ideal_hpf.bmp", out, N, N);
    printf("ideal_hpf.bmp saved (256x256, radius=%d).\n", RADIUS);
