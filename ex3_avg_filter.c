...
/* ── Exercise 3: 1/9 Average Filter (Low-pass) ── */
uint8** average_filter_3x3(uint8** input) {
    int y, x, dy, dx, sum;
...
int main(void) {
    ...
    uint8** output = average_filter_3x3(lena);
    write_bmp("output_3_avg_filter.bmp", output, IMG_WIDTH, IMG_HEIGHT);
    free_2d(lena);
    free_2d(output);
    printf("output_3_avg_filter.bmp 생성 완료\n");
    return 0;
}
