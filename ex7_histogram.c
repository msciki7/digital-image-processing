...
/* ── Exercise 7: Histogram Equalization ── */
uint8** histogram_equalization(uint8** input) {
    int y, x, i;
    ...
    /* 4) 매핑 */
    for (i = 0; i < 256; i++)
        mapping[i] = clip_to_byte((int)(cdf[i] * 255.0 + 0.5));

    /* 5) 적용 */
    for (y = 0; y < IMG_HEIGHT; y++)
        for (x = 0; x < IMG_WIDTH; x++)
            out[y][x] = mapping[input[y][x]];
...
int main(void) {
    ...
    uint8** output = histogram_equalization(lena);
    write_bmp("output_7_histogram.bmp", output, IMG_WIDTH, IMG_HEIGHT);
    free_2d(lena);
    free_2d(output);
    printf("output_7_histogram.bmp 생성 완료\n");
    return 0;
}
